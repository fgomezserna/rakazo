...
describe("host clipboard paste bridge", () => {
  ...
  it("ships the bridge next to embed.html in the computer image", () => {
    ...
    expect(embed).toMatch(/rakazo-novnc\.bundle\.js/);
    expect(embed).not.toMatch(/<script type="module">/);
    expect(embed).toMatch(/rakazo-novnc/);
    expect(entry).toMatch(/addEventListener\("connect"/);
    expect(start).toMatch(/clipboard-bridge\.js/);
    expect(supervisor).toMatch(/"clipboard-bridge\.js"/);
    expect(supervisor).toMatch(/"novnc-entry\.js"/);
  });
});
